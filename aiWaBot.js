...
import {
    ...
    checkDatabase,  // Certifique-se de importar a função corretamente
    openAitextWABot,
...
const webhookWa = async (m, wa, sessionId) => {
    try {
        ...
        const replyInGroup = bot.reply_in_groups > 0 ? true : false
        ...
        if (useMyAPI && !myOwnAPI) {
            const session = getSession(sessionId);
            await sendMessage(session, senderJid, { text: "**WARNING** You have selected to use your own API however your API KEYS are not there in your panel" });
        ...
        const passAPI = useMyAPI > 1 ? myOwnAPI : getAPI[0]?.openai_keys
